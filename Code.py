...
def resize_picture(work_image, margin_height = 30, margin_width = 30, half_matrix_size = 2):
    ...
    up_border = best_border_coord_up(work_image, margin_height = margin_height, half_matrix_size = half_matrix_size)
    down_border = best_border_coord_down(work_image, margin_height = margin_height, half_matrix_size = half_matrix_size)
    left_border = best_border_coord_left(work_image, margin_width = margin_width, half_matrix_size = half_matrix_size)
    right_border = best_border_coord_right(work_image, margin_width = margin_width, half_matrix_size = half_matrix_size)
    new_work_image = work_image[up_border[0]:down_border[0], left_border[1]:right_border[1]]
        ...
        while (done == False):
            ...
            for n in range(i-half_matrix_size,i+half_matrix_size+1):
                ...
                for m in range(dist-half_matrix_size, dist+half_matrix_size+1):
                    ...
                    around_dist[small_matrix_index_m, small_matrix_index_n] = work_image[m, n]

            if (np.array_equal(around_dist, ones_matrix) or dist == N1 - (half_matrix_size+1)):
                done = True
            else:
                dist = dist + 1
    ...
    for i in range(half_matrix_size, N1 - half_matrix_size):
        ...
        while (done == False):
            ...
            for n in range(i - half_matrix_size, i + half_matrix_size + 1):
                ...
                for m in range(dist - half_matrix_size, dist + half_matrix_size + 1):
                    small_matrix_index_m = m % matrix_size
                    around_dist[small_matrix_index_n, small_matrix_index_m] = work_image[n, m]

            if (np.array_equal(around_dist, ones_matrix) or dist == N2 - (half_matrix_size+1)):
                done = True
            else:
                dist = dist + 1
        distances[i - half_matrix_size,1] = dist
...
def mass_center(work_image):
    ...
    while (vertical_center_found == False):
        ...
        if ((right_mass == left_mass) or (v_nb_iter > 20)):
            ...
            new_vertical_index = vertical_index + ((abs(old_vertical_index - vertical_index))/2)
    ...
    while (horizontal_center_found == False):
        ...
        if ((up_mass == down_mass) or (h_nb_iter > 20)):
            ...
            new_horizontal_index = horizontal_index + ((abs(old_horizontal_index - horizontal_index)) / 2)
...
def connexity(work_image, half_matrix_size = 3):
    ...
    while ((start_point_reached == False) and (connexity == True)):
        ...
        if (it > 10):
            for i in range(current_point[0]-half_matrix_size, current_point[0]+half_matrix_size+1):
                for j in range(current_point[1]-half_matrix_size, current_point[1]+half_matrix_size+1):
        ...
        if (start_point_inside_current_matrix == True):
            ...
            for i in range(current_point[0]-half_matrix_size, current_point[0]+half_matrix_size+1):
                small_matrix_j = 0
                for j in range(current_point[1]-half_matrix_size, current_point[1]+half_matrix_size+1):
                    around_current_point[small_matrix_i, small_matrix_j] = work_image[i, j]
            ...
            if (mass_of_border >= 0):
                ...
                if ((current_point[0] <= mid_row) and (current_point[1] > mid_col)):
                    ...
                    next_point = [current_point[0] + row_to_add, current_point[1] + col_to_add]
                                ...
                                if (around_current_point[j, 0] == 2):
                                    ...
                                    for k in range(matrix_size):
                                        if (around_current_point[0, k] == 2):
                                            ...
                                            for l in range(matrix_size):
                                                if (around_current_point[l, matrix_size - 1] == 2):
                        ...
                        if (around_current_point[i, 0] == 2):
                            ...
                            for j in range(matrix_size):
                                if (around_current_point[0, j] == 2):
                                    ...
                                    for k in range(matrix_size):
                                        if (around_current_point[k, matrix_size - 1] == 2):
                                            row_to_add = k - half_matrix_size
                                            col_to_add = matrix_size - 1 - half_matrix_size
                                        elif ((row_to_add == 0) & (col_to_add == 0)):
                                            for l in range(matrix_size):
                                                if (around_current_point[matrix_size - 1, l] == 2):
                                                    row_to_add = matrix_size - 1 - half_matrix_size
                                                    col_to_add = l - half_matrix_size
                    next_point = [current_point[0] + row_to_add, current_point[1] + col_to_add]
                        ...
                        if (around_current_point[0, i] == 2):
                            ...
                            for j in range(matrix_size):
                                if (around_current_point[j, matrix_size - 1] == 2):
                                    ...
                                    for k in range(matrix_size):
                                        if (around_current_point[matrix_size - 1, k] == 2):
                                            row_to_add = matrix_size - 1 - half_matrix_size
                                            col_to_add = k - half_matrix_size
                                        elif ((row_to_add == 0) & (col_to_add == 0)):
                                            for l in range(matrix_size):
    ...
    if (matrix_4.shape != reverse_matrix_2.shape):
        if (matrix_4.shape[0] < reverse_matrix_2.shape[0]):
            if (matrix_4.shape[1] < reverse_matrix_2.shape[1]):
                ...
                new_matrix_4 = np.pad(matrix_4, ((row_to_add, 0), (col_to_add, 0)), mode = 'constant', constant_values=(0, 0))
                sub_matrix = np.subtract(new_matrix_4, reverse_matrix_2)
            else:
                row_to_add = reverse_matrix_2.shape[0] - matrix_4.shape[0]
                col_to_add =  matrix_4.shape[1] - reverse_matrix_2.shape[1]

                new_matrix_4 = np.pad(matrix_4, ((row_to_add, 0), (0, 0)), mode = 'constant', constant_values=(0, 0))
                new_reverse_matrix_2 = np.pad(reverse_matrix_2, ((0, 0), (col_to_add, 0)), mode = 'constant', constant_values=(0, 0))
            ...
            if (matrix_4.shape[1] < reverse_matrix_2.shape[1]):
                ...
                new_matrix_4 = np.pad(matrix_4, ((0, 0), (col_to_add, 0)), mode='constant', constant_values=(0, 0))
                new_reverse_matrix_2 = np.pad(reverse_matrix_2, ((row_to_add, 0), (0, 0)), mode='constant', constant_values=(0, 0))
                sub_matrix = np.subtract(new_matrix_4, new_reverse_matrix_2)
                ...
                new_reverse_matrix_2 = np.pad(reverse_matrix_2, ((row_to_add,0), (col_to_add, 0)), mode='constant', constant_values=(0, 0))
    ...
    if (matrix_1.shape != reverse_matrix_3.shape):
        if (matrix_1.shape[0] < reverse_matrix_3.shape[0]):
            if (matrix_1.shape[1] < reverse_matrix_3.shape[1]):
                ...
                new_matrix_1 = np.pad(matrix_1, ((row_to_add1, 0), (0, col_to_add1)), mode = 'constant', constant_values=(0, 0))
                ...
                new_matrix_1 = np.pad(matrix_1, ((row_to_add1, 0), (0, 0)), mode = 'constant', constant_values=(0, 0))
                new_reverse_matrix_3 = np.pad(reverse_matrix_3, ((0, 0), (0, col_to_add1)), mode = 'constant', constant_values=(0, 0))
            ...
            if (matrix_1.shape[1] < reverse_matrix_3.shape[1]):
                ...
                new_matrix_1 = np.pad(matrix_1, ((0, 0), (0, col_to_add1)), mode='constant', constant_values=(0, 0))
                new_reverse_matrix_3 = np.pad(reverse_matrix_3, ((row_to_add1, 0), (0, 0)), mode='constant', constant_values=(0, 0))
                sub_matrix2 = np.subtract(new_matrix_1, new_reverse_matrix_3)
                ...
                new_reverse_matrix_3 = np.pad(reverse_matrix_3, ((row_to_add1, 0), (0, col_to_add1)), mode='constant', constant_values=(0, 0))
...
if __name__ == '__main__':
    ...
    images = load_images()
    image = mpimg.imread("data/melanoma_17.jpg")
    glob_res = []
    res = [3, 0, 0, 0, 0, 0]
    ...
    show_image(new_work_image, "Cropped image")
